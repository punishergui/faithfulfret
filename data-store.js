...
db.exec(`CREATE TABLE IF NOT EXISTS song_playlists (
  song_id INTEGER NOT NULL,
  playlist_id INTEGER NOT NULL,
  created_at INTEGER NOT NULL,
  PRIMARY KEY (song_id, playlist_id)
)`);
db.exec(`CREATE TABLE IF NOT EXISTS video_song_links (
  video_id INTEGER PRIMARY KEY,
  song_id INTEGER NOT NULL,
  created_at INTEGER NOT NULL
)`);
ensureColumn('modules', 'level_id', 'INTEGER');
ensureColumn('sessions', 'song_id', 'INTEGER');
...
db.exec('CREATE INDEX IF NOT EXISTS idx_song_playlists_song ON song_playlists(song_id)');
db.exec('CREATE INDEX IF NOT EXISTS idx_song_playlists_playlist ON song_playlists(playlist_id)');
db.exec('CREATE INDEX IF NOT EXISTS idx_video_song_links_song ON video_song_links(song_id)');
...
function coerceSession(input = {}) {
  return {
    ...
    song_id: n(input.song_id ?? input.songId),
  };
}
...
Q = {
  upsertSession: db.prepare(`INSERT INTO sessions (id,date,title,durationMinutes,youtubeId,focusTag,notes,createdAt,bpm,dayNumber,focus,mood,win,checklist,links,videoId,song_id)
    VALUES (:id,:date,:title,:durationMinutes,:youtubeId,:focusTag,:notes,:createdAt,:bpm,:dayNumber,:focus,:mood,:win,:checklist,:links,:videoId,:song_id)
    ON CONFLICT(id) DO UPDATE SET
      ...
      checklist=excluded.checklist,links=excluded.links,videoId=excluded.videoId,song_id=excluded.song_id`),
...
const deleteSession = (id) => {
  ...
  run('DELETE FROM session_song WHERE session_id = ?', id);
...
const listTrainingVideos = (filters = {}) => {
  ...
  const sql = `SELECT training_videos.*, vsl.song_id AS linked_song_id, rs.title AS linked_song_title${selectProgress}${selectAttachmentCounts}
    FROM training_videos
    LEFT JOIN video_song_links vsl ON vsl.video_id = training_videos.id
    LEFT JOIN repertoire_songs rs ON rs.id = vsl.song_id${joinProgress}
    ${where.length ? `WHERE ${where.join(' AND ')}` : ''}
    ORDER BY training_videos.updatedAt DESC, training_videos.id DESC`;
  return db.prepare(sql).all(...values);
};

const getTrainingVideo = (id) => one(`SELECT training_videos.*, vsl.song_id AS linked_song_id, rs.title AS linked_song_title
  FROM training_videos
  LEFT JOIN video_song_links vsl ON vsl.video_id = training_videos.id
  LEFT JOIN repertoire_songs rs ON rs.id = vsl.song_id
  WHERE training_videos.id = ?`, Number(id));
...
const deleteTrainingVideo = (id) => {
  const tx = db.transaction((targetId) => {
    ...
    run('DELETE FROM video_song_links WHERE video_id = ?', targetId);
...
const deleteVideoPlaylist = (id) => {
  const tx = db.transaction((playlistId) => {
    ...
    run('DELETE FROM song_playlists WHERE playlist_id = ?', playlistId);
    run(`DELETE FROM timeline_events WHERE entity_id LIKE ?`, `song_playlist:%:${playlistId}`);
...
const getVideoPlaylistRollupCounts = (playlistIds = []) => {
  const targetIds = Array.from(new Set((playlistIds || []).map((id) => Number(id)).filter(Boolean)));
  if (!targetIds.length) return {};

  const rows = all(`
    SELECT
      COALESCE(playlist_id, playlistId) AS playlist_id,
      COALESCE(item_type, 'video') AS item_type,
      COALESCE(video_id, videoId) AS video_id,
      child_playlist_id
    FROM video_playlist_items
  `);
  ...
  rows.forEach((row) => {
    ...
    itemsByPlaylist.get(playlistId).push(row);
  });

  const memo = new Map();
  const collectVideos = (playlistId, visiting = new Set()) => {
    ...
    const items = itemsByPlaylist.get(id) || [];
    items.forEach((item) => {
      if (String(item.item_type) === 'playlist') {
        const childId = Number(item.child_playlist_id);
        if (!childId) return;
        const nestedVideos = collectVideos(childId, visiting);
        nestedVideos.forEach((videoId) => videos.add(videoId));
        return;
      }
  ...
  return targetIds.reduce((acc, playlistId) => {
    acc[playlistId] = collectVideos(playlistId).size;
    return acc;
  }, {});
};
...
const deleteTimelineEventsByEventType = (eventType) => {
  const safeEventType = String(eventType || '').trim();
  if (!safeEventType) return 0;
  return run('DELETE FROM timeline_events WHERE event_type = ?', safeEventType).changes || 0;
};
const resetBadges = () => {
  const clearedBadges = run('DELETE FROM badge_unlocks').changes || 0;
  const clearedTimelineBadgeEvents = deleteTimelineEventsByEventType('badge_unlocked');
  return {
    clearedBadges,
    clearedTimelineBadgeEvents,
  };
};
...
const deleteRepertoireSong = (id) => {
  ...
  run('DELETE FROM song_playlists WHERE song_id = ?', songId);
  run('DELETE FROM video_song_links WHERE song_id = ?', songId);
  deleteFeedEventsByEntity('song', songId);
...
const listSongPlaylists = (songId) => db.prepare(`SELECT sp.*, vp.name AS playlist_name, vp.description AS playlist_description
  FROM song_playlists sp
  JOIN video_playlists vp ON vp.id = sp.playlist_id
  WHERE sp.song_id = ?
  ORDER BY vp.name COLLATE NOCASE ASC`).all(Number(songId));

const listPlaylistSongs = (playlistId) => db.prepare(`SELECT sp.*, rs.title, rs.artist, rs.status
  FROM song_playlists sp
  JOIN repertoire_songs rs ON rs.id = sp.song_id
  WHERE sp.playlist_id = ?
  ORDER BY rs.title COLLATE NOCASE ASC`).all(Number(playlistId));

const linkSongPlaylist = (songId, playlistId) => {
  const now = Date.now();
  run('INSERT OR IGNORE INTO song_playlists (song_id, playlist_id, created_at) VALUES (?,?,?)', Number(songId), Number(playlistId), now);
  return one('SELECT * FROM song_playlists WHERE song_id = ? AND playlist_id = ?', Number(songId), Number(playlistId));
};

const unlinkSongPlaylist = (songId, playlistId) => {
  const result = run('DELETE FROM song_playlists WHERE song_id = ? AND playlist_id = ?', Number(songId), Number(playlistId));
  deleteFeedEventsByEntityKey(`song_playlist:${Number(songId)}:${Number(playlistId)}`);
  return result;
};

const getVideoSongLink = (videoId) => one(`SELECT vsl.video_id, vsl.song_id, vsl.created_at, rs.title, rs.artist, rs.status
  FROM video_song_links vsl
  LEFT JOIN repertoire_songs rs ON rs.id = vsl.song_id
  WHERE vsl.video_id = ?`, Number(videoId));

const linkSongVideo = (songId, videoId) => {
  const now = Date.now();
  run(`INSERT INTO video_song_links (video_id, song_id, created_at)
    VALUES (@video_id, @song_id, @created_at)
    ON CONFLICT(video_id) DO UPDATE SET song_id = excluded.song_id, created_at = excluded.created_at`, {
    video_id: Number(videoId),
    song_id: Number(songId),
    created_at: now,
  });
  return getVideoSongLink(videoId);
};

const unlinkSongVideo = (videoId) => {
  const targetId = Number(videoId);
  const linked = getVideoSongLink(targetId);
  const result = run('DELETE FROM video_song_links WHERE video_id = ?', targetId);
  if (linked?.song_id) deleteFeedEventsByEntityKey(`song_video:${Number(linked.song_id)}:${targetId}`);
  return result;
};
