...
(function() {
  const routes = [
    ...
    { pattern: /^#\/training\/all(?:\?.*)?$/,      handler: () => Pages.TrainingAll.render() },
    { pattern: /^#\/training\/skills$/,           handler: () => Pages.TrainingSkills.render() },
    { pattern: /^#\/training\/skills\/([a-z0-9-]+)$/, handler: (m) => Pages.TrainingSkillGroup.render(m[1]) },
    { pattern: /^#\/training\/songs(?:\?.*)?$/,    handler: () => Pages.TrainingSongs.render() },
    { pattern: /^#\/training\/song\/(\d+)$/,      handler: (m) => Pages.TrainingSong.render(m[1]) },
    { pattern: /^#\/training\/providers$/,        handler: () => Pages.TrainingProviders.render() },
    { pattern: /^#\/training\/admin$/,            handler: () => Pages.TrainingAdmin.render() },
    { pattern: /^#\/training\/videos\/(\d+)\/edit$/, handler: (m) => Pages.ResourceVideosEdit.render(m[1]) },
    { pattern: /^#\/training\/videos\/(\d+)$/,      handler: (m) => Pages.ResourceVideoDetail.render(m[1]) },
    { pattern: /^#\/training\/videos(?:\?.*)?$/,     handler: () => Pages.ResourceVideosList.render() },
    { pattern: /^#\/resources\/videos(?:\/.*)?$/,     handler: () => navigate('#/training/videos') },
