...
After deploy, verify `#/training/videos` song-link UX matches playlist cards: linked videos show a clickable song pill (`Title — Artist`), action button toggles **Link Song**/**Edit Link**, and modal supports link/change/unlink without any extra `Linked` label text.
...
After deploy, verify top-level playlist cards always show a thumbnail: use direct second-level video thumbnail when present, otherwise recurse through the first nested playlist chain, then fall back to placeholder if no videos exist anywhere under that playlist.
After deploy, verify `#/training/playlists` card headers keep **Link to Song** pinned at the top-right without thumbnail overlap, and metric tiles (**Videos/Watched/Mastered/Duration**) use larger readable values in both light and dark themes.
After deploy, verify the old playlist summary text line (`X videos • duration • type • difficulty`) no longer renders above metrics, and both top-level and nested playlist cards show **Link to Song** + metric tiles; confirm link modal supports search, link/change, unlink, and persists after reload.
...
After deploy, verify **Link to Song** for videos works in both playlist detail rows and Training Video Library cards (open modal, search/select, unlink/change), linked song badge appears, and links persist after refresh + export/import.
After deploy, verify Training Video Library cards keep the category pill(s) and **LINK SONG/CHANGE SONG** action on the same metadata row (pill left, action right) in both grid and list views, including long titles and narrow widths.
...
# SQL safety deploy check: with `DEBUG_SQL=1` temporarily enabled, hit `/api/stats`, `/api/timeline?limit=5&offset=0`, and `/api/feed?limit=10&offset=0`; verify logs print SQL + params and no `Too many parameter values were provided` errors appear.
...
# Dashboard hero deploy check: verify the Latest badge card renders in the right hero column (near Streak Restore) on desktop and stacks below greeting content on mobile without overlap.
# Dashboard timeline defaults deploy check: save multiple filters (for example Session + Training + Resource), navigate away and back, then refresh; the same pills stay active and feed results match saved defaults (not gear-only unless explicitly saved).
...
# Badges reset deploy check: run Reset badges in Settings and verify `/api/badges` returns `[]` plus `/api/feed?limit=50&offset=0` has zero items where `type` is `badge` (old badge cards/events must disappear without redeploy).
...
# Settings tabs deploy check: verify Settings hero includes Gear-style pill tabs (Theme/General/Timeline/Motivation/Data/Advanced), only one section is visible at a time, `#/settings?tab=theme` deep-links correctly, and last selected tab persists after refresh.
...
After deploy, verify Compact view hides all price-checking UI/summary blocks and keeps only core info (thumb, name, owned/wishlist pill, used count, last used, manual link) without overflow.
After deploy, verify List view renders true single-row entries (thumb, left-aligned truncated name, used count, last used, owned/wishlist pill, manual link), with category text removed inside rows/cards.
